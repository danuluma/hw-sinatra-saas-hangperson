...
describe HangpersonGame do
  ...
  describe 'game status' do
    ...
    it 'should be win when all letters guessed' => true do
      guess_several_letters(@game, 'dog')
      expect(@game.check_win_or_lose).to eq(:win)
    end
    it 'should be lose after 7 incorrect guesses' => true do
      guess_several_letters(@game, 'tuvwxyz')
      expect(@game.check_win_or_lose).to eq(:lose)
    end
    it 'should continue play if neither win nor lose' => true do
